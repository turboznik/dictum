//! Portable mode support for Dictum.
//!
...
fn is_valid_portable_marker(path: &std::path::Path) -> bool {
    std::fs::read_to_string(path)
        .map(|s| s.trim().starts_with("Dictum Portable Mode"))
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn test_valid_magic_string_enables_portable() {
        ...
        write!(f, "Dictum Portable Mode").unwrap();
    ...
    #[test]
    fn handy_portable_marker_does_not_enable_dictum_portable_mode() {
        let dir = std::env::temp_dir().join("dictum_test_handy_marker");
        std::fs::create_dir_all(&dir).unwrap();
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
    ...
    fn test_magic_string_with_whitespace_enables_portable() {
        ...
        let marker = dir.join("portable");
        let mut f = std::fs::File::create(&marker).unwrap();
        writeln!(f, "  Dictum Portable Mode").unwrap();
        assert!(is_valid_portable_marker(&marker));
        std::fs::remove_dir_all(dir).unwrap();
    }
