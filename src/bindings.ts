...
async changeRealtimeChunkDurationSetting(duration: number) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_realtime_chunk_duration_setting", { duration }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type TranscriptionMode = "standard" | "realtime" | "stream" | "batch_stream"
